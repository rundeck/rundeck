      ...
      <div class="col-xs-12">
        <div class="card">
          <div class="card-content">
            <div>
            <h3 class="card-title">
              Verb <span style="font-size: smaller">/ Plugins</span>
            </h3>
  ...
  methods: {
    search () {
      ...
      axios({
        ...
        url: `${this.rdBase}verb/repository/artifacts/search`,
    ...
    install(repoName, pluginId) {
      ...
      axios({
        method: 'post',
        headers: {'x-rundeck-ajax': true},
        url: `${this.rdBase}verb/repository/install/${pluginId}`,
        withCredentials: true
      }).then((response) => {
  ...
  mounted () {
    if (window._rundeck && window._rundeck.rdBase) {
      ...
      axios({
        method: 'get',
        headers: { 'x-rundeck-ajax': true },
        url: `${this.rdBase}verb/repository/artifacts/list`,
        withCredentials: true
      }).then((response) => {
        if (response.data) {
