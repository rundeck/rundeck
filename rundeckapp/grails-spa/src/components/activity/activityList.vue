  ...
  <div class="activity-list">
    <section class="section-space-bottom">
      <span>
        <span v-if="pagination.total > 0 && pagination.total > pagination.max" class="text-muted">
          ...
          <span
            v-if="!loading"
          >{{pagination.offset+reports.length}}</span>
        ...
        <a :href="activityHref">
          <span
            class="summary-count"
            :class="{ 'text-primary': pagination.total < 1, 'text-info': pagination.total > 0 }"
      ...
      <div class="pull-right">
        ...
        <span v-if="auth.deleteExec && pagination.total>0 && showBulkDelete">
          <span v-if="bulkEditMode">
            ...
            <span class="btn btn-default btn-xs" @click="showBulkEditCleanSelections=true">
    ...
    >
      ...
      <div slot="footer">
        ...
        >{{$t('Only shown executions')}}</button>
        <button
    ...
    <modal v-model="showBulkEditConfirm" id="bulkexecdelete" :title="$t('Bulk Delete Executions')">
      ...
      <div slot="footer">
        <btn @click="showBulkEditConfirm=false">{{$t('cancel')}}</btn>
        <btn type="danger" @click="performBulkDelete" data-dismiss="modal">{{$t('Delete Selected')}}</btn>
      </div>
    </modal>
    ...
    >
      ...
      <div v-if="!bulkEditProgress">
        <p v-if="bulkEditResults && bulkEditResults.requestCount > 0" class="text-info">
          <i18n path="bulkresult.attempted.text" tag="p">
            <strong>{{bulkEditResults.requestCount}}</strong>
          </i18n>
        </p>
        <p v-if="bulkEditResults && bulkEditResults.successCount > 0" class="text-success">
          <i18n path="bulkresult.success.text" tag="p">
            <strong>{{bulkEditResults.successCount}}</strong>
          </i18n>
        </p>
        <p v-if="bulkEditResults && bulkEditResults.failedCount > 0" class="text-warning">
          <i18n path="bulkresult.failed.text" tag="p">
            <strong>{{bulkEditResults.failedCount}}</strong>
          </i18n>
        </p>
        <div
          v-if="bulkEditResults && bulkEditResults.failures && bulkEditResults.failures.length > 0"
        >
    ...
    <div class="card-content-full-width">
      <table class="table table-hover table-condensed">
          ...
          >
            ...
            <td class="dateStarted date" v-tooltip="runningStatusTooltip(exec)">
              <progress-bar
                ...
                type="default"
                label
                :label-text="$t('job.execution.starting.0',[runningStartedDisplay(exec.dateStarted.date)])"
              />
              <progress-bar
            ...
            <td
              ...
              v-if="exec.job"
              v-tooltip="(exec.job.group ? exec.job.group + '/' +exec.job.name : '')"
            >{{exec.job.name}}</td>
            ...
            <td class="eventtitle adhoc" v-if="!exec.job" colspan="2">{{exec.description}}</td>
        ...
        <tbody class="since-count-data autoclickable" @click="reload" v-if="sincecount>0">
          <tr>
            <td colspan="6" class="text-center">{{ $tc('info.newexecutions.since.0', sincecount) }}</td>
          </tr>
        </tbody>
        <tbody class="history-executions" v-if="reports.length > 0">
          ...
          >
            ...
            <td
              class="right date"
              v-tooltip.bottom="$t('info.completed.0',[jobCompletedFormat(rpt.dateCompleted)])"
            >
              <span v-if="rpt.dateCompleted">
                <span class="timeabs">{{rpt.dateCompleted | moment('from','now')}}</span>
                <span title>
                  <span class="text-primary">
                    <i18n path="in.of" default="in"/>
                  </span>
                  <span
                    class="duration"
                    data-bind="text: durationHumanize()"
                  >{{rpt.duration | duration('humanize')}}</span>
                </span>
              </span>
            </td>
            ...
            <td class="eventtitle" :class="{job:rpt.jobId,adhoc:!rpt.jobId}">
              <span
                ...
                v-tooltip="(rpt.jobGroup ? rpt.jobGroup + '/' +rpt.jobName: '')"
              >{{rpt.jobName}}</span>
              <span v-else>{{rpt.executionString}}</span>
              <span
                v-if="query.jobIdFilter && rpt.jobId && query.jobIdFilter !==rpt.jobId"
              ...
              <span
                v-if="rpt.jobDeleted"
                class="text-primary"
              >{{$t('job.has.been.deleted.0',[rpt.jobName])}}</span>
            ...
            <td class="text-right">
              <a :href="rpt.executionHref">#{{rpt.executionId}}</a>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="reports.length < 1 " class="loading-area">
      <span class="text-secondary" v-if="!loading && !loadError">{{$t('results.empty.text')}}</span>
...
import {
  ...
  ExecutionListRunningResponse,
  Execution
} from "ts-rundeck/dist/lib/models";
  ...
  methods: {
    ...
    runningStatusTooltip(exec: Execution) {
      if (
        ...
        return (this as any).$t("job.execution.starting.0", [
        ...
        if (exec.job && exec.job.averageDuration) {
          ...
          return (this as any).$t("info.started.expected.0.1", [
            startmo.fromNow(),
            expected.fromNow()
          ]);
        }
    ...
    async bulkDeleteExecutions(ids: string[]) {
      ...
      try {
        this.bulkEditResults = await rundeckContext.rundeckClient.executionBulkDelete(
          { ids }
        );
    ...
    async loadRunning() {
      ...
      try {
        this.running = await rundeckContext.rundeckClient.executionListRunning(
          this.projectName
        );
