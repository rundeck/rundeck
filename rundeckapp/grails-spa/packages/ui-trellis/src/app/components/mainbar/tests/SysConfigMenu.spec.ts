...
describe("SysConfigMenu", () => {
  ...
  const mountSysConfigMenu = async (
    ...
    const wrapper = mount(SysConfigMenu, {
      ...options,
      global: {
        components: {
          Dropdown,
  ...
  describe("Data Loading", () => {
    ...
    it("should handle case when DOM element does not exist or contains invalid data", async () => {
      // Mount the component
      const wrapper = await mountSysConfigMenu({ props: { links: [] } });
  ...
  describe("Link Filtering and Sorting", () => {
    ...
    it("should handle an empty links array", async () => {
  ...
  describe("Link and Submenu Rendering", () => {
    it("should correctly render simple links", async () => {
      // Mount the component
      const wrapper = await mountSysConfigMenu({
        props: { links: simpleLinks },
      });
  ...
  describe("Edge Cases", () => {
    it("should handle empty links array", async () => {
      ...
      expect(wrapper.vm.links).toEqual([]);
      expect(wrapper.vm.enabledLinks).toEqual([]);

      // Verify that only the system header is rendered
      expect(wrapper.find(".dropdown-header").text()).toBe("System");
      expect(wrapper.findAll("li:not(.dropdown-header)")).toHaveLength(0);
    });
