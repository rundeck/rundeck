  ...
  <span class="optview">
    ...
    >
      ...
      <span
        ...
        :title="(option.description||'')+(option?.required ? ' (Required)' : '')"
        >{{ option.name }}</span
      >
      <span class="">
        <span :title="displayDefaultValue" :class="{truncatedtext:displayDefaultValue.length>20}">
...
export default defineComponent({
  ...
  computed:{
    ...
    displayDefaultValue(){
      return this.option.secureInput && this.option.defaultValue?'****':this.option.defaultValue||''
    },
    displayDefaultValueTruncated(){
