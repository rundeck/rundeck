    ...
    <aside>
      <div class="flex--none bg-grey-100">
        <span class="title"
          ...
          {{ $t("message_webhookPageTitle") }}</span
        >
      </div>
      ...
      <div class="flex--none bg-grey-100">
        <button
          ...
          :class="{
            'btn-primary': isProjectLoaded && noWebhooksForProject && !curHook,
          }"
          @click="handleAddNew"
        >
          ...
          >
            <Tab :index="0" :title="$t('message_webhookTabGeneral')">
              <div class="wh-edit__body">
                <div class="form-group">
                  <div class="card card-accent">
                    ...
                    <div class="card-content">
                      ...
                      <div class="help-block">
                        {{ $t("message_webhookGenerateSecretCheckboxHelp") }}
                      </div>
                      <div class="checkbox">
                        <input
                          id="wh-authtoggle"
                      ...
                      <div v-if="showRegenButton">
                        ...
                        <span
                          ...
                          style="color: var(--color-red); margin-left: 5px"
                        >
                          {{ $t("message_webhookRegenClicked") }}</span
                        >
                      </div>
                      <div v-if="hkSecret">
                        <label
                          style="color: var(--color-red); margin-top: 5px"
                          >{{ $t("message_webhookSecretMessageHelp") }}</label
                        >
                ...
                <div class="card">
                  <div class="card-content">
                    <div class="form-group">
                      <label>{{ $t("message_webhookNameLabel") }}</label
                      ><input
                        v-model="curHook.name"
                        class="form-control"
                        @change="input"
                      />
                    </div>
                    ...
                    <div class="form-group">
                      <label>{{ $t("message_webhookRolesLabel") }}</label
                      ><input
                        v-model="curHook.roles"
                        class="form-control"
                        @change="input"
                      />
                      <div class="help-block">
            ...
            >
              <div class="wh-edit__body">
                ...
                <div
                  v-if="selectedPlugin && showPluginConfig"
                  class="new-section"
                >
                  <div
...
export default defineComponent({
  ...
  data() {
    const projectName = window._rundeck
  ...
  methods: {
    confirmAuthToggle() {
      ...
      if (this.curHook.useAuth) {
        this.$confirm({
          title: "Confirm",
          content:
            "Are you sure you want to remove this webhook authorization string?",
        })
          .then(() => {
            //do nothing
          })
    ...
    toggleUrl(hookId, show) {
      if (show)
        document.getElementById("whc-" + hookId).style.display = "block";
    ...
    setSelectedPlugin(preserve, type) {
      ...
      this.selectedPlugin = {
        type: this.curHook.eventPlugin,
        config: this.curHook.config,
      };
      if (!preserve) {
        this.setValidation(true);
      }
      ...
      if (!this.curHook.eventPlugin.artifactName) {
        console.error(
          "Webhook: eventPlugin missing artifactName",
          this.curHook.eventPlugin,
        );
        return;
      }

      console.log(
        "Webhook: Loading plugin description for",
      ...
      getServiceProviderDescription(
        ...
        .then((data) => {
          ...
          console.log(
            "Webhook: showPluginConfig set to",
            this.showPluginConfig,
          );
        })
        .catch((error) => {
    ...
    async handleSave() {
      ...
      if (!data || data.err) {
        ...
        this.select(
          this.webhookStore.webhooksByUuid.get(data.uuid),
          showAuthString,
        );
      }
    },
