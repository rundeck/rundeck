...
const actions = {
  setInstallStatusOfPluginsVisibility({ commit }, showWhichPlugins) {
    commit("SET_PLUGIN_VISIBILITY_BY_INSTALL_STATUS", showWhichPlugins);
  },
  setSupportTypeFilter({ commit }, filters) {
  ...
  installPlugin({ commit, dispatch }, properties) {
  ...
  uninstallPlugin({ commit, dispatch }, properties) {
    commit("SET_ERRORS", null);
    dispatch(
      "overlay/openOverlay",
      {
