    ...
    global: {
      ...
      stubs: {
        ...
        KeyStorageEdit: {
          ...
          template:
            '<div data-testid="mock-key-storage-edit">' +
...
const mountKeyStorageSelector = async (
  props = {},
...
describe("KeyStorageSelector", () => {
  ...
  it.each([
    ...
    "passes storageSelector %p prop to key storage view",
    async (storageFilter: string) => {
  ...
  it.each(["keys/some/path", "keys/another/path"])(
    "passes modelValue %p prop to key storage view",
  ...
  it.each([true, false])(
    "passes allowUpload %p prop to key storage view",
    async (allowUpload: boolean) => {
  ...
  it.each([true, false])(
    "passes readOnly %p prop to key storage view",
  ...
  it.each(["keys"])(
    "passes rootPath %p prop to key storage view",
  ...
  it.each(["myKey", "key2"])(
    "opens editor with key %p when KeyStorageView overwrite key button clicked",
  ...
  it.each(["myKey", "key2"])(
    "close modal and emit with key %p when KeyStorageView save button clicked",
    async (keyName: string) => {
      mockedStorageKeyGetMetadata.mockResolvedValue({
        resources: [
          {
