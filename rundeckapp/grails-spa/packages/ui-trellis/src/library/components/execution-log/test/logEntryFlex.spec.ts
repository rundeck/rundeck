...
describe("LogEntryFlex.vue", () => {
  ...
  it("renders correctly with default props", () => {
    ...
    expect(wrapper.find(".execution-log__content-text").text()).toBe(
      "Test log entry",
    );
  });
  ...
  it("renders with selected class when selected prop is true", () => {
    ...
    expect(wrapper.find(".execution-log__line").classes()).toContain(
      "execution-log__line--selected",
    );
  });
  ...
  it("adds line-wrap class when lineWrap is true", () => {
    const wrapper = createWrapper();
    expect(
      wrapper.find(".execution-log__content-text").classes(),
    ).not.toContain("execution-log__content-text--overflow");
  });

  it("adds overflow class when lineWrap is false", () => {
    const config = { ...defaultConfig, content: { lineWrap: false } };
    const wrapper = createWrapper({ config });
    expect(wrapper.find(".execution-log__content-text").classes()).toContain(
  ...
  it("correctly sets level class on content", () => {
    ...
    expect(wrapper.find(".execution-log__content").classes()).toContain(
      "execution-log__content--level-error",
    );
  });
  ...
  it("displays slim gutter when timestamps are visible but command is not", () => {
    ...
    const wrapper = createWrapper({ config });
    expect(wrapper.find(".execution-log__gutter").classes()).toContain(
