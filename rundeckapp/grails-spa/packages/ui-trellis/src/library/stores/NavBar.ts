...
export class NavBar {
  ...
  overflowOne() {
    const candidate = this.containerGroupItems("root", "main")
      .slice()
      .reverse()
      .shift();

    if (candidate) candidate.container = "overflow";
  }

  showOne() {
    const candidate = this.containerGroupItems("overflow", "main")
      .slice()
      .shift();

    if (candidate) candidate.container = "root";
  }
}
