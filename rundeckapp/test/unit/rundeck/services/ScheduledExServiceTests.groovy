...
class ScheduledExServiceTests {
    ...
    public void testDoValidateEmptyInput() {
            ...
            def params = [:]
            def results = service._dovalidate(params, testUserAndRolesContext('test', 'test'))
    ...
    public void testDoValidateNodedispatchIsBlank() {
        ...
        def results = testService._dovalidate(params, testUserAndRolesContext('test', 'test'))
        assertFalse(results.failed)
        assertNotNull(results.scheduledExecution)
        assertTrue(results.scheduledExecution instanceof ScheduledExecution)
        final ScheduledExecution execution = results.scheduledExecution
...
//        ms.demand.getMessage { key, data, locale -> key + ":" + data.toString() + ":" + locale.toString() }
    ...
    public void testDoValidateNotifications() {
        ...
        //test job with notifications
            ...
            def results = sec._dovalidate(params, testUserAndRolesContext('test', 'test'))
            if (results.scheduledExecution.errors.hasErrors()) {
                results.scheduledExecution.errors.allErrors.each {
                    System.err.println(it);
                }
...
//        sec.request.setAttribute("subject", subject)
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_dow_dom_combo() {
        def sec = new ScheduledExecutionService()
...
//            sec.scheduledExecutionService = sesControl.createMock()
            ...
            qtzControl.demand.scheduleJob { jobDetail, trigger -> new Date() }
            sec.quartzScheduler = qtzControl.createMock()
            def params = new ScheduledExecution(
                    scheduled: true,
                    crontabString: '0 21 */4 */4 */6 3 2010-2040',
                    useCrontabString: 'true',
                    jobName: 'monkey1',
                    project: 'testProject',
                    description: 'blah',
                    ...
                    workflow: new Workflow(
                            commands: [new CommandExec(adhocRemoteString: 'test command', adhocExecution: true)]
                    )
            )
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_dow_dom_combo2() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
...
//            sec.scheduledExecutionService = sesControl.createMock()
            ...
            def params = new ScheduledExecution(
                    scheduled: true,
                    crontabString: '0 21 */4 ? */6 ? 2010-2040',
                    useCrontabString: 'true',
                    jobName: 'monkey1',
                    project: 'testProject',
                    description: 'blah',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalidYearChar() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
            ...
            def params = new ScheduledExecution(
                    scheduled: true,
                    crontabString: '0 21 */4 */4 */6 ? z2010-2040',
                    useCrontabString: 'true',
                    workflow: new Workflow(
                            commands: [new CommandExec(adhocRemoteString: 'test command', adhocExecution: true)]
                    )
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_tooFewComponents() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_seconds() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_minutes() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_hour() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_dom() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_month() {
        def sec = new ScheduledExecutionService()
            def se = new ScheduledExecution(
                    jobName: 'monkey1',
                    project: 'testProject',
    ...
    public void testDoUpdateJobShouldNotSetInvalidCrontabString_invalid_dow() {
        def sec = new ScheduledExecutionService()
...
//            sec.scheduledExecutionService = sesControl.createMock()
    ...
    public void testDoUpdateWorkflowDataStrategy() {
        def sec = new ScheduledExecutionService()

            def se = new ScheduledExecution(jobName: 'monkey1', project: 'testProject', description: 'blah3')
            def workflow = new Workflow(threadcount: 1, keepgoing: true,strategy: 'node-first')
            def wfitem = new CommandExec(adhocExecution: true, adhocRemoteString: 'test command',)
            workflow.addToCommands(wfitem)
            se.workflow = workflow
            se.save()
...
//            sec.scheduledExecutionService = sesControl.createMock()
            ...
            def params = [id: se.id.toString(),
                          workflow: [
                                  'commands[0]': [adhocExecution: true, adhocRemoteString: 'test command2',],
                                  strategy: 'step-first',
                                  keepgoing: 'false'
                          ],
                          '_workflow_data': true]
            ...
            assertFalse(execution.errors.hasErrors())
            assertNotNull(execution.workflow)
            assertEquals 'step-first', execution.workflow.strategy
            assertEquals false, execution.workflow.keepgoing
    }
    public void testDoUpdateWorkflowSessionStrategy() {
        def sec = new ScheduledExecutionService()
...
//            sesControl.demand.getByIDorUUID {id -> return se }
//            sec.scheduledExecutionService = sesControl.createMock()
            def sessionWf = new Workflow(workflow)
            def params = [id: se.id.toString(),
                    '_sessionwf':true,
                    '_sessionEditWFObject':sessionWf,
                          workflow: [
                                  strategy: 'step-first',
                                  keepgoing: 'false'
                          ]
    ...
    public void testDoUpdateWorkflowJustStrategy() {
        def sec = new ScheduledExecutionService()
...
//            sec.scheduledExecutionService = sesControl.createMock()
    ...
    private void assertLoadJobs(String importOption, String uuidOption, String uuid1, String uuid2, String project1, String project2, Closure test) {
        ...
        def result = sec.loadJobs([upload], importOption,uuidOption, [:], testUserAndRolesContext('test', 'userrole,test'))
        assertNotNull result
        assertNotNull result.jobs
        assertNotNull result.errjobs
        assertNotNull result.skipjobs
