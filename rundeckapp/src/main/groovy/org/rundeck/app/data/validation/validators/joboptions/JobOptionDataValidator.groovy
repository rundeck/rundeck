...
class JobOptionDataValidator implements Validator {
    ...
    JobOptionDataValidator(FrameworkService frameworkService, UserService userService, JobData jobData) {
    ...
    void validate(Object target, Errors errors) {
        ...
        if(jobWasScheduled && opt.required && !(opt.defaultValue||opt.defaultStoragePath)){
            ...
            if(opt.realValuesUrl){
                try{
                    def userInfo = userService.getCurrentUserInfo()
                    def realUrl = opt.realValuesUrl.toExternalForm()
                    def urlExpanded = OptionDataUtil.expandUrl(frameworkService, opt, realUrl, jobData, null, userInfo)
