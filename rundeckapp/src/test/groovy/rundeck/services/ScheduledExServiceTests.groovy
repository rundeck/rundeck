...
//            getFrameworkFromUserSession { session, request -> return null }
    ...
    public void testDoUpdate() {
        ...
        def schedulesService = new MockFor(JobSchedulesService, false)

        def se = new ScheduledExecution(jobName: 'monkey1', project: 'testProject', description: 'blah')
        se.save()
        schedulesService.demand.persistSchedulesToJob{ uuid, schedules, shouldSchedule, project ->}
        schedulesService.demand.shouldScheduleExecution{ jobUUID -> false}
        sec.jobSchedulesService = schedulesService.proxyInstance()
        assertNotNull se.id
