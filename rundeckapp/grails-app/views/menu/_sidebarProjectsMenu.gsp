        ...
        <g:if test="${!params.nocreate}">
          <li>
            <g:link controller="framework" action="createProject">
              <span class="sidebar-mini"><i class="glyphicon glyphicon-plus"></i></span> <span class="sidebar-normal"><g:message code="page.home.new.project.button.label"/></span>
            </g:link>
          </li>
        </g:if>
      ...
      <g:each var="project" in="${projectSet}">
        <li>
          <g:link controller="menu" action="index" params="${selectParams + [project: project]}">
            <span class="sidebar-mini">${(labels?labels[project]:project).charAt(0)}</span> <span class="sidebar-normal">${labels?labels[project]:project}</span>
          </g:link>
        </li>
      </g:each>
    </nav>
