...
<div class="content">
	<div id="layoutBody">
						...
						<div class="card-content">
							<span class="text-h3" data-bind="if: loadedProjectNames()">
								<span data-bind="messageTemplate: projectNamesTotal, messageTemplatePluralize:true">
									<g:message code="page.home.section.project.title"/>|<g:message
											code="page.home.section.project.title.plural"/>
								</span>
							</span>
							...
							<div data-bind="if: projectCount() > 0 && loaded()">
								...
								<span class="h4">
									<span class="summary-count"
									      data-bind="css: { 'text-info': execCount()>0, 'text-strong': execCount()<1 }">
									...
									<span class="summary-count"
									      data-bind="css: { 'text-warning': totalFailedCount()>0, 'text-strong': totalFailedCount()<1 }">
		...
		<div class="container-fluid" data-bind="if: projectCount() == 0">
			<auth:resourceAllowed action="${AuthConstants.ACTION_CREATE}" kind="${AuthConstants.TYPE_PROJECT}"
			                      context="${AuthConstants.CTX_APPLICATION}" has="true">
				<div id="firstRun"></div>
			</auth:resourceAllowed>
		</div>
		...
		<div class="container-fluid" data-bind="if: projectCount()<1 && loadedProjectNames()">
			<div class="row">
				<auth:resourceAllowed action="${AuthConstants.ACTION_CREATE}" kind="${AuthConstants.TYPE_PROJECT}"
				                      context="${AuthConstants.CTX_APPLICATION}" has="false">
					<div class="col-sm-12">
						<div class="card">
							<div class="card-content">
								<div class="well">
									<g:set var="roles"
									       value="${request.subject?.getPrincipals(com.dtolabs.rundeck.core.authentication.Group.class)?.collect { it.name }?.join(", ")}"/>
									...
									<p>
										<g:message
												code="no.authorized.access.to.projects.contact.your.administrator.user.roles.0"
												args="[roles]"/>
									</p>
								</div>
							</div>
						...
						<div id="project-list" class="card-content">
							<div>
								<div class="input-group">
									...
									<input type="search" name="search"
									       ...
									       class="form-control input-sm" data-bind="value: search"/>
								...
								<div data-bind="if: filtered.enabledFiltersCount()>0 && loadedProjectNames()">
									<div class="alert alert-info">
										<span data-bind="messageTemplate: searchedProjectsCount(), messageTemplatePluralize:true, css: { 'text-info': searchedProjectsCount()>0, 'text-warning': searchedProjectsCount()<1 }">
											<g:message code="page.home.search.project.title"/>|<g:message
													code="page.home.search.project.title.plural"/>
										</span>
									</div>
								</div>
							</div>
							...
							<div data-bind="if: pagingEnabled()">
								<span class="text-muted" data-bind="if: paging.hasPages()">
									<span data-bind="text: paging.pageFirstIndex"></span>-<span
										data-bind="text: paging.pageLastIndex"></span>
										...
										<div class="col-sm-6 col-md-8">
											<a href="${g.createLink(action: 'index', controller: 'menu', params: [project: '<$>'])}"
											   ...
											   class="link-hover  text-inverse project_list_item_link link-quiet">
												...
												<span class="h5"
												      data-bind="if: $root.projectForName(project) && $root.projectForName(project).label">
													<div data-bind="text: $root.projectForName(project).label"></div>
												</span>
												<span class="h5"
												      data-bind="ifnot: $root.projectForName(project) && $root.projectForName(project).label">
													<span data-bind="text: project"></span>
												</span>

												<span class="h5"
												      data-bind="if: !$root.projectForName(project).executionEnabled()">
													<span class="text-base text-warning  has_tooltip"
													      data-placement="right" data-bind="bootstrapTooltip: true"
													      title="${message(code: 'project.execution.disabled')}">
														<i class="glyphicon glyphicon-pause"></i>
													</span>
												</span>
												<span class="h5"
												      data-bind="if: !$root.projectForName(project).scheduleEnabled()">
													<span class="text-base text-warning has_tooltip"
													      data-placement="right" data-bind="bootstrapTooltip: true"
													      title="${message(code: 'project.schedule.disabled')}">
												...
												<span data-bind="if: $root.projectForName(project)">
													<span class="text-secondary text-base"
													      data-bind="text: $root.projectForName(project).description"></span>
												</span>
											</a>
										</div>
											...
											<div data-bind="if: $root.projectForName(project)">
												<a data-bind="css: { 'text-secondary': $root.projectForName(project).execCount()<1 }, urlPathParam: project,  bootstrapPopover: true, bootstrapPopoverContentRef: '#exec_detail_'+project "
												   ...
												   data-popover-template-class="popover-wide popover-primary">
													<span class="summary-count "
													      data-bind="css: { 'text-info':$root.projectForName(project).execCount()>0 } ">
														...
														<span data-bind="if: $root.projectForName(project).loaded()">
															<span data-bind="if: $root.projectForName(project).execCount()>0">
																<span data-bind="text: $root.projectForName(project).execCount()"
																      class="text-h3"></span>
															</span>
												...
												<div data-bind="if: $root.projectForName(project).userCount()>0,attr: { 'id': 'exec_detail_'+project },"
												     style="display:none;">
													<span data-bind="if: $root.projectForName(project).execCount()>0">
													...
													<span data-bind="messageTemplate: $root.projectForName(project).execCount(), messageTemplatePluralize: true">
														<g:message code="Execution"/>|<g:message
																code="Execution.plural"/>
													</span>
													<g:message code="page.home.duration.in.the.last.day"/>
													<g:message code="by"/>
													<span class="text-info"
													      data-bind="text: $root.projectForName(project).userCount()">
													</span>
													...
													<a data-bind="urlPathParam: project "
													   ...
													   href="${g.createLink(
														...
														<span data-bind="messageTemplate: $root.projectForName(project).failedCount()">
															<g:message
																	code="page.home.project.executions.0.failed.parenthetical"/>
														</span>
													</a>
												</span>
											</div>
											...
											<div class="pull-right">
												<div class="dropdown-toggle-hover">
													...
													<ul class="dropdown-menu pull-right" role="menu">
														...
														<li class="divider"
														    data-bind="if: $root.projectForName(project).auth().admin"></li>
														<!-- ko if: $root.projectForName(project).auth().jobCreate -->
														<li>
															<a href="${g.createLink(controller: "scheduledExecution", action: "create", params: [project: '<$>'])}"
															   data-bind="urlPathParam: project">
														...
														<li>
															<a href="${g.createLink(controller: "scheduledExecution", action: "upload", params: [project: '<$>'])}"
															   data-bind="urlPathParam: project" class="">
