...
<g:set var="gkeys" value="${groupTree.sort{a,b->a.key<=>b.key}}"/>
<g:timerEnd key="gtx"/>
<g:each in="${gkeys}" var="group">
    <g:timerStart key="_groupTree.gsp-loop"/>
    <li class="expandComponentHolder">
        ...
        <g:timerEnd key="_groupTree.gsp-loop"/>
        <g:if test="${group.value.subs || group.value.jobs}">
    ...
    <g:if test="${currentJobs}">
        <g:timerStart key="_groupTree.gsp-jobslist"/>
        <li>
        ...
        <g:timerEnd key="_groupTree.gsp-jobslist"/>
    </g:if>
</ul>