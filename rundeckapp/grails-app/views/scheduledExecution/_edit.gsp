...
<div class="tab-pane active" id="tab_details" data-ko-bind="jobeditor">
    <section class="section-space-lg">
        %{--name--}%
        <div class="form-group ${g.hasErrors(bean: scheduledExecution, field: 'jobName', 'has-error')}"
            ...
            <g:render template="/common/modal"
                      model="[modalid: 'groupChooseModal', titleCode: 'job.edit.groupPath.choose.text']">
        ...
        <div class="form-group ${hasErrors(bean: scheduledExecution, field: 'description', 'has-error')}">
            <label for="description" class="${labelColClass}"><g:message
                    code="scheduledExecution.property.description.label"/></label>

            <div class="${fieldColSize}">
                <ul class="nav nav-tabs">

                    <li class="active"><a href="#desceditor" data-toggle="tab">Edit</a></li>
                    <li id="previewrunbook"
                        style="${wdgt.styleVisible(if: g.textHasMarker(text: scheduledExecution?.description, marker: ScheduledExecution.RUNBOOK_MARKER))}">
...
<g:set var="projectName"
       value="${scheduledExecution.project ? scheduledExecution.project.toString() : params.project ?: request.project ?: projects?.size() == 1 ? projects[0].name : ''}"/>
...
<div class="tab-pane" id="tab_workflow">
    <section id="optionsContent" class=" section-space-lg">
        <div class="job-editor-workflow-vue" id="job-editor-workflow-vue">
            <workflow-editor-section :event-bus="EventBus" :edit-mode="true" />
        </div>
    ...
    <section id="workflowContent" class="section-separator section-space-lg">
        <div class="form-group">
            ...
            <div class="${fieldColSize}" style="padding-top:1em;">
                <g:set var="editwf"
                       value="${session.editWF && session.editWF[scheduledExecution.id.toString()] ? session.editWF[scheduledExecution.id.toString()] : scheduledExecution.workflow}"/>
                <g:render template="/execution/execDetailsWorkflow"
                          model="${[workflow       : editwf, context: scheduledExecution, edit: true, error: scheduledExecution?.errors?.hasFieldErrors('workflow'), project: scheduledExecution?.project ?: (params.project ?: request.project) ?: projects?.size() == 1 ? projects[0].name : '',
...
<div class="tab-pane" id="tab_schedule">
    <section class="section-space-lg">
        ...
        <div class="form-group">
            ...
            <div class="${offsetColSize}" style="${wdgt.styleVisible(if: scheduledExecution?.scheduled)}"
                 id="scheduledExecutionEditCrontab">
        ...
        <div class="form-group" style="${wdgt.styleVisible(if: scheduledExecution?.scheduled)}"
             id="scheduledExecutionEditTZ">
            <div class="${labelColSize} control-label text-form-label">
...
<feature:enabled name="executionLifecyclePlugin">
    <g:if test="${executionLifecyclePlugins}">
        <g:set var="executionLifecyclePluginConfigMap"
               value="${scheduledExecution?.pluginConfigMap?.get('ExecutionLifecycle') ?: [:]}"/>
        <div class="tab-pane" id="tab_execution_plugins">
            ...
            <div class="list-group">
                <g:each in="${executionLifecyclePlugins}" var="plugin">
                    <g:set var="pluginKey"
                           value="${params.executionLifecyclePlugin?.type?.get(pluginType) ?: g.rkey()}"/>
                    ...
                    <g:set var="pluginConfig"
                           value="${params.executionLifecyclePlugins?.get(pluginKey)?.configMap ?: executionLifecyclePluginConfigMap[pluginType]}"/>
...
<div class="tab-pane" id="tab_other">
    <section class="section-space-lg">
        <div class="form-group">
            ...
            <div class="${fieldColSize}">
                <div class="radio radio-inline">
                    <g:radio id="log-level-normal" name="loglevel" value="INFO"
                             checked="${scheduledExecution?.loglevel != 'DEBUG'}"/>
                ...
                <div class="radio radio-inline">
                    <g:radio id="log-level-debug" name="loglevel" value="DEBUG"
                             checked="${scheduledExecution?.loglevel == 'DEBUG'}"/>
        ...
        <div class="form-group">
            ...
            <div class="${fieldColHalfSize}">
                ...
                <input type='text' name="maxMultipleExecutions"
                       value="${enc(attr: scheduledExecution?.maxMultipleExecutions)}"
        ...
        <div class="form-group ${hasErrors(bean: scheduledExecution, field: 'retry', 'has-error')}">
            <div class="${labelColSize} control-label">
                <label for="schedJobRetry"><g:message code="scheduledExecution.property.retry.label"
                                                      default="Retry"/></label>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="${fieldColShortSize}">
                ...
                <input type='text' name="logOutputThreshold"
                       value="${enc(attr: scheduledExecution?.logOutputThreshold)}"
            ...
            <div class="${fieldColShortSize}">
                <div class="radio">
                    <g:radio id="logOutputThresholdAction" name="logOutputThresholdAction" value="halt"
                             checked="${!scheduledExecution?.logOutputThresholdAction || scheduledExecution?.logOutputThresholdAction == 'halt'}"/>
                    <label for="logOutputThresholdAction"
                           title="${message(code: "scheduledExecution.property.logOutputThresholdAction.halt.description")}">
                        <g:message code="scheduledExecution.property.logOutputThresholdAction.halt.label"/>
                    </label>
                </div>

                <div class="input-group">
                    <g:helpTooltip code="scheduledExecution.property.logOutputThresholdAction.halt.description"
                                   placement="left"/>
                    <input type='text' name="logOutputThresholdStatus"
                           value="${enc(attr: scheduledExecution?.logOutputThresholdStatus)}"
                ...
                <div class="radio">
                    <g:radio id="logOutputThresholdActionTruncateAndContinue" name="logOutputThresholdAction"
                             value="truncate" checked="${scheduledExecution?.logOutputThresholdAction == 'truncate'}"/>
                    <label for="logOutputThresholdActionTruncateAndContinue"
                           title="${message(code: "scheduledExecution.property.logOutputThresholdAction.truncate.description")}">
        ...
        %{--uuid--}%
        <div class="form-group ${hasErrors(bean: scheduledExecution, field: 'uuid', 'has-error')}"
