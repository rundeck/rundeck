...
<div class=" wfnodestate container" data-node="${node}" data-template="${template}"
     style="${wdgt.styleVisible(unless: template)}"
    ...
    <div class="row wfnodeoverall  " data-node="${node}" data-bind-attr="data-node:nodename">
        <g:render template="wfstateNodeStepTemplate"
                  model="[node: node, state: laststate, ident: lastident, overall: true]"/>
    </div>
        ...
        <div class="wfnodesteps">
            <g:if test="${template}">
                <div data-template="step" style="display: none;" class="wfnodestep"
                     data-bind-attr="data-stepctx:stepctx">
                    <div class="row"
                ...
                <g:each in="${nodestate?.stepStateMap.keySet().sort()}" var="ident" status="i">
                    ...
                    <div class="wfnodestep" data-node="${node}" data-stepctx="${stepctx}">
                        <div class="row"
                             ...
                             data-node="${node}">
                            <g:render template="wfstateNodeStepTemplate"
                                      model="[node: node, state: state, ident: ident]"/>
                        </div>

                        <div class="row">
                            <div class="col-sm-offset-3 col-sm-9 wfnodeoutput" data-node="${node}"
