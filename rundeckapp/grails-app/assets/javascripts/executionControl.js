...
var FollowControl = Class.create({
  ...
  bindActions: function (elem) {
    ...
    $(elem).select('.opt_append_top_true').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setOutputAppendTop(true);
      });
    });
    $(elem).select('.opt_append_top_false').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setOutputAppendTop(false);
      });
    });
    ...
    $(elem).select('.opt_auto_scroll_true').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setOutputAutoscroll(e.checked);
      });
    });
    $(elem).select('.opt_group_output').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setGroupOutput(e.checked);
      });
    });
    $(elem).select('.opt_collapse_ctx').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setCollapseCtx(e.checked);
      });
    });
    $(elem).select('.opt_show_final').each(function (e) {
      e.onclick = null;
      Event.observe(e, 'click', function (evt) {
        obj.setShowFinalLine(e.checked);
      });
    });
    $(elem).select('.opt_last_lines_dec').each(function (e) {
      e.onmousedown = null;
      Event.observe(e, 'mousedown', function (evt) {
        Event.stop(evt);
        obj.modifyLastlines(-5);
      });
    });
  ...
  readyMode: function () {
    ...
    if (this.targetElement && $(this.targetElement)) {
      ...
      if (this.tailmode) {
        ...
        $(this.targetElement).select('.out_setmode_browse').each(function (e) {
  ...
  genDataRowBrowse: function (data, tbl) {
    ...
    console.log('could this be the one')
    this.configureDataRow(tr, data, ctxid);

    var tableContainer = jQuery(tbl).parent();
    var scrollHeight = jQuery(tableContainer).context.scrollHeight;
    jQuery(tableContainer[0]).scrollTop(scrollHeight);
  ...
  configureDataRow: function (tr, data, ctxid) {
    console.log('config row data', tr, data, ctxid)
    ...
    if (shownode && data.node) {
      ...
      // jQuery('').scrollTop(jQuery('#messages')[0].scrollHeight);
  ...
  docancel: function () {
    ...
    return jQuery.ajax({
      ...
      error: function (jqxhr, status, err) {
        obj.updatecancel({
          error: "Failed to kill Job: " + (jqxhr.responseJSON && jqxhr.responseJSON.error ? jqxhr.responseJSON.error : err)
        });
      }
  ...
  doincomplete: function () {
    ...
    return jQuery.ajax({
      ...
      error: function (jqxhr, status, err) {
        obj.updatecancel({
          error: "Failed to mark Job as incomplete: " + (jqxhr.responseJSON && jqxhr.responseJSON.error ? jqxhr.responseJSON.error : err)
        });
      }
    }).success(_createAjaxReceiveTokensHandler('exec_cancel_token'));
  },
