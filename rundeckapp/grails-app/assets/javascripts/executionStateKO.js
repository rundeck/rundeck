  ...
  self.getParentJobStepInfoForStepctx = function (parentctx, callback) {
    if (parentctx) {
      ...
      self.loadJob(self.jobId, function (job) {
        callback(new WorkflowStepInfo(self, '', {
          id: self.jobId,
          type: 'job',
          job: job
        }));
...
function RDNode(name, steps, flow) {
  ...
  self.steps = ko.observableArray([]).extend({
    rateLimit: 200
  });
...
function NodeFlowViewModel(workflow, outputUrl, nodeStateUpdateUrl, multiworkflow, data) {
  ...
  self.nodes = ko.observableArray([]).extend({
    rateLimit: 500
  });
  ...
  self.totalSteps = ko.pureComputed(function () {
  ...
  self.showOutput = function (nodestep) {
    ...
    var ctrl = new FollowControl(null, null, {
      ...
      appLinks: {
        tailExecutionOutput: self.followOutputUrl
      },
  ...
  self.updateNodes = function (model) {
    ...
    self.pendingNodeSteps(model.completed ? {
      '_other': 0
    } : self.countPendingStepsForNodes(model, model.allNodes));
  ...
  self.loadStateForNode = function (node) {
    var obj = self;
    return jQuery.ajax({
      url: _genUrl(self.nodeStateUpdateUrl, {
        node: node.name
      }),
      dataType: 'json',
