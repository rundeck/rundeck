 ...
 * "scheduledExecutor" runs delayed tasks for retrying at a later time
...
class LogFileStorageService implements InitializingBean,ApplicationContextAware{
    ...
    boolean getConfiguredStorageFailureCancel(){
        if (grailsApplication.config?.rundeck?.execution?.logs?.fileStorage?.containsKey('cancelOnStorageFailure')) {
            return grailsApplication.config?.rundeck?.execution?.logs?.fileStorage?.cancelOnStorageFailure in [true, 'true']
        }
        true
    }
    /**
