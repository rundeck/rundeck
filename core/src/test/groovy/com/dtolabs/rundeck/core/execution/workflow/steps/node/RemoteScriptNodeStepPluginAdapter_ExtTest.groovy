...
class RemoteScriptNodeStepPluginAdapter_ExtTest extends Specification {
    ...
    def "per-value quoting preserves template semicolon for safe values"() {
        given:
            ...
            sharedContext.merge(
                    ...
                    com.dtolabs.rundeck.core.dispatcher.DataContextUtils.context("data", [home_dir: '/workspace'])
            )
            sharedContext.merge(
                    com.dtolabs.rundeck.core.dispatcher.ContextView.global(),
                    com.dtolabs.rundeck.core.dispatcher.DataContextUtils.context("job", [execid: 'exec123'])
            )
            ...
            def script = Mock(com.dtolabs.rundeck.plugins.step.FileExtensionGeneratedScript) {
                getCommand() >> ['cd', '${data.home_dir}/${job.execid};'].toArray()
            }
            def scriptUtils = Mock(com.dtolabs.rundeck.core.execution.workflow.steps.node.impl.ScriptFileNodeStepUtils)
        ...
        when:
            ...
            framework.frameworkServices.getExecutionService() >> Mock(ExecutionService) {
                executeCommand(_, _, node) >> { args ->
                    ...
                    Mock(com.dtolabs.rundeck.core.execution.service.NodeExecutorResult) { isSuccess() >> true }
                }
            }
            RemoteScriptNodeStepPluginAdapter_Ext.executeRemoteScript(context, node, script, 'exec123', 'testProvider', scriptUtils)
    ...
    def "per-value quoting does not quote unquoted refs"() {
        given:
            ...
            sharedContext.merge(
                    com.dtolabs.rundeck.core.dispatcher.ContextView.global(),
                    com.dtolabs.rundeck.core.dispatcher.DataContextUtils.context("option", [port: '80 && whoami'])
            )
