...
main() {
    ...
    docker-compose -f selenium-docker-compose.yml run \
        ...
        selenium "apt-get -y --no-install-recommends install google-chrome-stable &&npm install && npm run bootstrap && ./selenium/bin/deck test -s selenium -u http://rundeck:4440 -h --s3-upload --s3-base ${S3_BASE}" || RET=$?
