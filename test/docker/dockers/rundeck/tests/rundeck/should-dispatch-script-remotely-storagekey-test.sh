...
it_should_dispatch_script_remotely_stored_key() {
    ...
    bash -c "dispatch -p $RUNDECK_PROJECT -f -F ${REMOTE_NODE}-stored -s /tests/rundeck/test-dispatch-script.sh" > test.output
...
it_should_dispatch_script_remotely_dos_lineendings_stored_key() {
    # Run the script file on the remote node
    bash -c "dispatch -p $RUNDECK_PROJECT -f -F ${REMOTE_NODE}-stored -s /tests/rundeck/test-dispatch-script-dos.sh" > test.output
    test "$(head -n1 test.output)" = "Succeeded queueing adhoc"
    tail -n +3 test.output > test2.output
...
it_should_dispatch_script_remotely_with_args_stored_key() {
    bash -c "dispatch -p $RUNDECK_PROJECT -f -F '${REMOTE_NODE}-stored' -s /tests/rundeck/test-dispatch-script.sh -- arg1 arg2"> test.output
    test "$(head -n1 test.output)" = "Succeeded queueing adhoc"
    tail -n +3 test.output > test2.output
...
it_should_dispatch_url_remotely_stored_key() {
    bash -c "dispatch -p $RUNDECK_PROJECT -f -F '${REMOTE_NODE}-stored' -u file:/tests/rundeck/test-dispatch-script.sh -- arg1 arg2"> test.output
    test "$(head -n1 test.output)" = "Succeeded queueing adhoc"
    tail -n +3 test.output > test2.output
