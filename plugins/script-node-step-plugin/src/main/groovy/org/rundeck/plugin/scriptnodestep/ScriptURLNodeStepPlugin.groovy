...
@Plugin(service = ServiceNameConstants.WorkflowNodeStep, name = ScriptURLNodeStepPlugin.PROVIDER_NAME)
@PluginDescription(title = "Script URL Node Step", description = "Verify and validate design")
class ScriptURLNodeStepPlugin implements NodeStepPlugin  {
    public static final Logger logger                      = LoggerFactory.getLogger(ScriptURLNodeStepPlugin.class.getName());
    public static final String PROVIDER_NAME = "script-url-node-step-plugin";

    @PluginProperty(title = "URL",
            description = "Url",
            required = true)
    private String path;
    ...
    ) throws NodeStepException
    {
        ...
        final String finalUrl = expandUrlString(path, sharedContext, node.getNodename());
        final URL url;
        try {
