...
- **`rc-backport-<version>-applied`** - applied by the script once a PR is confirmed part of a tagged (or about-to-be-tagged) RC. Authoritative "already applied" signal, checked before the cherry-pick trailer. **If you ever manually cherry-pick a labeled PR into the RC lineage yourself, push the branch or tag it lives on FIRST, then add this label** (never before pushing - see Scenario 4 for why) - the next run doesn't know a PR is in unless the label or trailer says so, and will otherwise try to re-apply it. The script itself only ever adds this label **after `--push` and after the tag genuinely exists** - never speculatively - since a local-only tag can vanish with the checkout while the label would remain, durable and visible to everyone, on GitHub.
...
**Once `v6.2.0-rc3` is genuinely created and pushed, `rescue/v6.2.0-rc3` is deleted** (best-effort) - both locally and, with `--push`, on origin - since the tag is now the durable record and the branch would just be confusing leftover state otherwise. This only happens when this run actually resumed from the branch: a fresh, conflict-free run never touches `rescue/<tag>` itself (a conflict would have exited earlier, before reaching cleanup), so a same-named branch found at this point in a fresh run was never inspected or used - most likely something else's, created during this run's own execution window - and is left alone rather than guessed at. Without `--push`, only the local branch is deleted - a remote copy (from an earlier `--push` run) is left alone, since this run never touched origin.
